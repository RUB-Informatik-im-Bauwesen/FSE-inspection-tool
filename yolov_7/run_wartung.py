...
def run(
    ...
    names = ['January', 'October', 'November', 'December', 'February', '2020', '2021', '2022', '2023', '2024', '2025', '2026', 'March', 'April', 'May', 'June', 'July', 'August', 'September']
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for (*xyxy, conf, cls) in det:
                    ...
                    cropped_image_path = save_dir.parent / f'{class_name}_results.jpg'
            ...
            # Extract year and month from the file name using regular expressions
            month_year_match = re.search(r'(\d{4})-(\d{2})', p.stem)
            year = month_year_match.group(1) if month_year_match else None
            month = month_year_match.group(2) if month_year_match else None
            ...
            if save_json and json_data_list:
                ...
                for data in json_data_list:
                    object_data = {
                        ...
                        'comment': f"{data['type']}",
                        'bounding_box': {
                            'x_center': data['bounding_box']['x_center'],
                            'y_center': data['bounding_box']['y_center'],
                    ...
                    # Check if the class name contains letters (month) or only digits (year)
                    if data['type'].isalpha():
                        month_data = object_data
                    elif data['type'].isdigit():
                        year_data = object_data

                    # Create the Inspection Tag entry with nested Year and Month entries
                inspection_tag = {
                    "Inspection Tag": {
                        "guid": year_data['guid'],
                        "bounding_box": year_data['bounding_box'] if year_data else month_data['bounding_box'],
                        "input image name": p.name,
                    }
                }

                # Add Year and Month entries if available
                if year_data:
                    inspection_tag["Inspection Tag"]["Year"] = year_data['comment']

                if month_data:
                    inspection_tag["Inspection Tag"]["Month"] = month_data['comment']

                json_data = {
                    'info': {
                        'description': 'Result of Service-207',
                        'version': '0.0.2'
                    },
                    'result': {
                        'types': [year, month] if year and month else [],
                        'objects': [inspection_tag],
                    }
                }
